...
In order to improve performance on typical OLTP workloads, which is what many of our users have, we engineered many optimizations in TiDB’s parser and cost-based optimizer to speed up Point Select operations. When you access your data via Unique Key or Primary Key using 2.1, you will see a 50% performance improvement over 2.0. You can find the details and reproduce [the benchmark here](https://github.com/pingcap/docs/blob/master/benchmark/sysbench-v3.md). 
...
Back when we released 2.0 in April, we published a TPC-H Scale 50 benchmark [comparing 2.0 with 1.0](https://github.com/pingcap/docs/blob/master/benchmark/tpch.md), which showed improvements on all queries, including ones that couldn’t even run on 1.0. However, query 17 and 18 were quite slow.
