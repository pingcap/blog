...
> **Note:** Either the size limit or the number of rows limit needs to consider the overhead of TiDB encoding and the extra transaction Key. **It is recommended that the number of rows of each transaction is less than 200**; otherwise, the limit might be exceeded or the performance is bad.
