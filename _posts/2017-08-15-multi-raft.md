...
excerpt: The goal of TiKV is to support 100 TB+ data and it is impossible for one Raft group to make it, we need to use multiple Raft groups, which is called Multi-raft. 
---
...
The problem of Range is that a Region may probably become a performance hotspot due to frequent operations. But we can use PD to schedule these Regions onto better machines and provide Followers to share read pressure.
...
The value of both `RAFT_INIT_LOG_TERM` and `RAFT_INIT_LOG_INDEX` is 5 (as long as larger than 1). In TiKV, there are several ways to create a Peer:
...
We have registered a base Raft Tick in mio and call it every 100ms. Store will traverse all Peers, call the corresponding `RawNode tick` function at a time to drive Raft.
...
The Server layer is TiKV’s network layer. Currently, TiKV uses mio to handle the whole network while the network protocol is customized:
...
We use header + body for any message: body is the actual message data and uses protobuf encoding; header is a 2-byte magic value, `0xdaf4`, version number, the length of the message and its unique ID.

Mio is the encapsulated epoll on Linux. Therefore, users who are familiar with epoll will find it quite convenient to use mio for network development. Below is the simple process:

+ Bind a port, generate a `TcpListener` object, and register to mio.

+ Handle the callback of `TcpListener on_readable`, call `socket TcpStream` generated by the `accept` function, register to mio. Subsequently, we will use this `TcpStream` to interact with the client.

+ `TcpStream` handles the callback of `on_readable` or `on_writable`.

Meanwhile, Server receives messages from outside through mio’s `notify`. For example, `Transport` implemented by TiKV directly sends this message to Server through channel when Peer is calling `send`. Then it handles in `notify`, finds the corresponding Store connection and sends to the remote Store.

As for the sending of snapshot, Server will create a new link and directly use a thread to synchronize. In this way, the code logic will be much easier and you don’t need to handle too much asynchronous IO logic. When the receiving end receives a message, it will first of all check the type of the message. If the message is snapshot, it will accept and then directly sends the data it has received to the relevant thread of snapshot and writes into the corresponding snapshot file. Otherwise, the receiving end will directly dispatch to the corresponding handle logic. For more information, please refer to the `on_conn_msg` function of Server.

Since Server handles the network IO and its logic is simple, I’m not going to elaborate. However, given that currently TiKV uses the customized network protocol, it is not good for interacting with other external clients. Besides, it doesn’t have the excellent traits like pipeline or stream. Thus, we will shift to gRPC in the future.
